...
class CRM_Event_ActionMapping extends \Civi\ActionSchedule\Mapping {
  ...
  public function createQuery($schedule, $phase, $defaultParams) {
    ...
    // AMGA Customization
    // This takes the current start date of the scheduled reminder and passes it through our custom hook,
    // where we add clauses based on scheduled reminders.
    $queryParams = array(
      'dateClause' => $query['casDateField'],
    );
    CRM_Emailsystem_Hook::alterScheduleReminderQuery($queryParams, $schedule);
    $query['casDateField'] = $queryParams['dateClause'];
    if (!empty($queryParams['whereClause'])) {
      $query->where($queryParams['whereClause']);
    }
    // AMGA Customization end
